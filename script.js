...
const App = { 
    ...
    Features: { 
        ...
        async triggerChangePasswordAction() { 
            ...
            try { 
                ...
                App.Toast.show(err.message, "danger"); 
            } 
        }, 
        ...
        async openSpecificFriendPopupObrolan(friendEmail, friendName) { 
            ...
            mainArea.innerHTML = ` <div class="popup-msg-stream" id="popup-msg-stream-viewport"></div> <div class="popup-chat-input-row"> <div class="emoji-panel-trigger" onclick="App.Features.toggleEmojiPanelDOM('chat-emoji-panel')"><span class="material-symbols-outlined">mood</span></div> <div class="emoji-popup-box" id="chat-emoji-panel" style="display:none;">${chatEmojiHTML}</div> <input type="text" class="form-input" placeholder="Tulis pesan..." id="popup-chat-input-field" onkeypress="if(event.key==='Enter') App.Features.sendPopupChatCloudAction()"> <button class="icon-btn" style="background:var(--primary); color:white; width:34px; height:34px;" onclick="App.Features.sendPopupChatCloudAction()"><span class="material-symbols-outlined" style="font-size:16px;">send</span></button> </div> `; 
            try { 
                const myCurrentName = App.ProfileState.getCurrentName(); 
                const { data: messages, error } = await supabaseClient .from('posts') .select('*') .eq('image', 'private_chat_type') .or(`and(author.eq.${myCurrentName},role.eq.${friendName}),and(author.eq.${friendName},role.eq.${myCurrentName})`) .order('created_at', { ascending: true }); 
                if (error) throw error; 
            ...
            try { 
                ...
                container.innerHTML = uniqueUsers.map(u => { 
                    ...
                    if (statusPertemanan === 'approved') { 
                        buttonHTML = `<button class="btn btn-secondary" style="padding:6px 14px; font-size:0.8rem;" onclick="App.Features.toggleFriendAction('${username}', 'approved', ${targetDbId})">Hapus Teman</button>`; 
                        ...
                        buttonHTML = `<button class="btn btn-primary" style="padding:6px 14px; font-size:0.8rem;" onclick="App.Features.toggleFriendAction('${username}', 'none', null)">Tambah</button>`; 
                    } 
        ...
        async toggleFriendAction(friendUsername, currentStatus, targetDbId) { 
            const myEmail = localStorage.getItem('ns_user_email') || ''; 
            if(!myEmail) return; 
            try { 
        ...
        async renderPosts() { 
            ...
            try { 
                ...
                stream.innerHTML = filteredPosts.map(p => { 
                    ...
                    let parsedComments = []; 
                    try { 
                        if(p.role && p.role.startsWith('[')) parsedComments = JSON.parse(p.role); 
                    ...
                    return ` <div class="glass-card post-card" id="post-card-container-${pId}"> <div class="post-header"> <div class="post-author"> <img src="${p.avatar || 'https://i.pravatar.cc/150?img=12'}" class="post-avatar"> <div class="post-meta"> <h4 onclick="App.Features.redirectToTargetFriendProfile('${p.author}')">${p.author}</h4> <span style="font-size: 0.75rem; color: var(--text-muted); font-weight: 500;">${formattedFullTimestamp}</span> </div> </div> ${postMenuConfigHTML} </div> <div class="post-content">${p.content || ''}</div> ${attachedMediaHTML} <div class="post-actions-bar"> <div class="post-action-item ${isLikedActive}" onclick="App.Features.toggleLike(${pId}, ${p.likes || 0}, ${p.dislikes || 0})"> <span class="material-symbols-outlined">thumb_up</span> <span>${p.likes || 0}</span> </div> <div class="post-action-item ${isDislikedActive}" onclick="App.Features.toggleDislike(${pId}, ${p.likes || 0}, ${p.dislikes || 0})"> <span class="material-symbols-outlined">thumb_down</span> <span>${p.dislikes || 0}</span> </div> <div class="post-action-item" onclick="App.Features.togglePostCommentsVisibility(${pId})"> <span class="material-symbols-outlined">comment</span> <span>${commentCountValue}</span> </div> <div class="post-action-item" onclick="App.Features.openSocialMediaShareSheetModal(${pId}, '${(p.content || "").replace(/'/g, "\\'")}')"> <span class="material-symbols-outlined">share</span> </div> </div> <div class="comments-section" id="comments-section-container-${pId}"> <div class="comments-stream">${commentsListHTML}</div> <div class="comment-row"> <div class="emoji-popup-box" id="comment-panel-${pId}" style="display:none; bottom:45px; left:0;">${commentEmojiHTML}</div> <div class="emoji-panel-trigger" onclick="App.Features.toggleEmojiPanelDOM('comment-panel-${pId}')"><span class="material-symbols-outlined">mood</span></div> <input type="text" class="form-input" placeholder="Tulis komentar..." style="padding:10px 14px; border-radius:20px; font-size:0.85rem;" id="comment-input-field-${pId}"> <button class="icon-btn" style="background:var(--primary); color:white; width:34px; height:34px; border-radius:50%; flex-shrink:0;" onclick="App.Features.submitPostCommentAction(${pId})"><span class="material-symbols-outlined" style="font-size:16px;">send</span></button> </div> </div> </div> `; }).join(''); 
            } catch (err) { console.error(err); } 
        }, 
        async createPost() { 
        ...
        showNotifications() { 
            ...
            if (liveNotifications.length === 0) { 
                App.Modal.open("Notifikasi Workspace", `<p style="padding:12px; text-align:center; color:var(--text-muted);">Tidak ada pemberitahuan baru.</p>`); 
                return; 
            } 
            ...
            const notiHTML = liveNotifications.map(item => { 
                ...
                let tagLabel = 'FEED'; 
                let tagColorClass = 'meta-tag-feed'; 
                let actionsHTML = '';
                
                if(item.type === 'chat') { 
                    tagLabel = 'PESAN'; 
                    tagColorClass = 'meta-tag-chat'; 
                } 
                if(item.type === 'admin') { 
                    tagLabel = 'SUPER ADMIN'; 
                    tagColorClass = 'meta-tag-admin'; 
                } 
                if(item.type === 'like') { 
                    tagLabel = 'LIKE / DISLIKE'; 
                    tagColorClass = 'meta-tag-like'; 
                } 
                if(item.type === 'comment') { 
                    tagLabel = 'KOMENTAR'; 
                    tagColorClass = 'meta-tag-comment'; 
                } 
                if(item.type === 'friend_request') {
                    ...
                    actionsHTML = `
                        <div style="display:flex; gap:8px; margin-top:10px;">
                            ...
                            <button class="btn btn-secondary" style="padding:4px 12px; font-size:0.75rem; border-radius:6px; color:var(--danger);" onclick="App.Features.rejectFriendRequestAction(${item.id}, ${item.friendRequestId})">Tolak</button>
                        </div>
                    `;
                }
            ...
            App.Modal.open("Notifikasi Workspace", `<div style="padding:4px 0;">${notiHTML}</div>`); 
            document.getElementById('modal-ok-btn').style.display = 'inline-flex'; 
        }, 
        ...
        handleAvatarSelection(event) { 
            ...
            reader.onload = function (e) { 
                ...
                img.onload = function () { 
                    ...
                    if(p) p.src = localStorage.getItem('ns_user_avatar_base64'); 
                }; 
            }; 
        }, 
        saveNameSettings() { 
            ...
            if(res.success) { 
                App.Toast.show("Profil disimpan!", "success"); 
        ...
        async submitPostCommentAction(postId) { 
            ...
            try { 
                const { data: postData } = await supabaseClient.from('posts').select('role, author').eq('id', postId).single(); 
                let commentArray = []; 
                try { 
                    if(postData && postData.role && postData.role.startsWith('[')) commentArray = JSON.parse(postData.role); 
                } catch(ex) { commentArray = []; } 
    ...
    Theme: { 
        ...
        toggle() { 
            ...
            localStorage.setItem('ns_theme', n); 
        } 
    }, 
    ...
    Network: { 
        listen() { 
            window.addEventListener('online', ()=>{
                const netEl = document.getElementById('network-text');
                if(netEl) netEl.innerText='Online';
            }); 
            window.addEventListener('offline', ()=>App.Toast.show("Offline","danger")); 
        } 
    }, 
    ...
    UI: { 
        renderGlobalBannerAdIfExists() { 
            ...
            if(savedAdText && savedAdText.trim() !== "") { 
                container.innerHTML = ` <div class="global-workspace-ad-banner"> <span class="material-symbols-outlined" style="color:var(--primary); animation: pulse 2s infinite;">campaign</span> <div style="font-size:0.85rem; font-weight:600; color:var(--text-main);"><marquee scrollamount="4">${savedAdText}</marquee></div> </div> `; 
        ...
        bindGlobalEvents() { 
            document.getElementById('btn-toggle-sidebar')?.addEventListener('click', () => { 
                ...
                if(window.innerWidth <= 768) { 
                    l.classList.toggle('sidebar-mobile-open'); 
            ...
            document.getElementById('btn-refresh')?.addEventListener('click', () => { 
                ...
                App.Toast.show("Diperbarui","success"); 
            }); 
        ...
        toggleChatPopup(shouldOpen) { 
            const chatBox = document.getElementById('floating-chat-popup-box'); 
            if(!chatBox) return; 
            if(shouldOpen) { 
        ...
        updateGlobalMessageNotificationBadgeCount() { 
            let totalUnread = 0; 
            for (const email in unreadMessageCounters) { totalUnread += unreadMessageCounters[email]; } 
            const sidebarCounter = document.getElementById('main-sidebar-chat-counter'); 
            const mobileCounter = document.getElementById('mobile-bn-chat-counter'); 
            if(totalUnread > 0) { 
                if(sidebarCounter) { 
                    sidebarCounter.innerText = totalUnread; 
                    sidebarCounter.style.display = 'flex'; 
                } 
                if(mobileCounter) { 
                    mobileCounter.innerText = totalUnread; 
        ...
        syncGlobalAvatarAndName() { 
            ...
            const avatar = App.ProfileState.getCurrentAvatar(); 
            const bio = App.ProfileState.getCurrentRoleBioStatus(); 
            ...
            if(sidebarRoleEl) sidebarRoleEl.innerText = bio; 
