...
const App = {
    ...
    ProfileState: {
        ...
        getCurrentRoleBioStatus() {
            // Cek jika Mastrisnocom@gmail.com jadikan super admin otomatis
    ...
    Auth: {
        ...
        async login(e) {
            ...
            if (error) { App.Toast.show(error.message, "danger"); return; }
            localStorage.setItem('ns_logged_in', 'true');
        ...
        async signup(e) {
            ...
            if (error) { App.Toast.show(error.message, "danger"); return; }
            App.Modal.open("Verifikasi Email", "Kami telah mengirimkan tautan konfirmasi ke email Anda.");
        ...
        showApp() {
            ...
            setTimeout(() => { auth.style.display = 'none'; app.classList.add('active'); }, 400);
        }
    },
    ...
    Views: {
        ...
        profil() {
            ...
            // MASTRISNOCOM@GMAIL.COM JADIKAN SUPER ADMIN BISA SEMUANYA
            const myEmail = (localStorage.getItem('ns_user_email') || '').toLowerCase();
            const isSuperAdmin = (myEmail === 'mastrisnocom@gmail.com');
            
            // FORM PASANG IKLAN DIBAWAH HEADER PADA PROFIL SUPERADMIN
            let adFormHTML = '';
            if (isSuperAdmin) {
                adFormHTML = `
                    <div class="glass-card" style="margin-top: 24px; text-align: left; border: 1px solid var(--primary);">
                        ...
                        <div class="form-group" style="margin-bottom: 14px;">
                            <input type="text" class="form-input" id="admin-ad-text-input" placeholder=" " value="${localStorage.getItem('ns_ad_text_global') || ''}">
                            <label class="form-label">Teks Pengumuman / Iklan Berjalan</label>
                        </div>
                        <button class="btn btn-primary btn-full" onclick="App.Features.saveAdminGlobalAdBannerAction()">Pasang & Publikasikan Iklan</button>
                    </div>
                `;
            }
    ...
    Features: {
                ...
                <div style="text-align:left;">
                    ...
                    <div style="display:flex; justify-content:space-between; align-items:center;">
                        <div style="display:flex; gap:12px;">
                            <button class="icon-btn" style="color:var(--primary)" onclick="document.getElementById('composer-img-input').click()"><span class="material-symbols-outlined">image</span></button>
                            <button class="icon-btn" style="color:var(--warning)" onclick="document.getElementById('composer-file-input').click()"><span class="material-symbols-outlined">attachment</span></button>
                        ...
                        <button class="btn btn-primary" onclick="App.Features.createPost()">Terbitkan Postingan</button>
                    </div>
                </div>
            `;
        ...
        // ==========================================================================
        // TIMELINE NEWS FEED MENAMPILKAN POSTINGAN DENGAN BENAR
        // ==========================================================================
        async renderPosts() {
        ...
        async createPost() {
            ...
            if(!txt || (!txt.value.trim() && !composerAttachedImageBase64 && !composerAttachedFileBase64)) {
            ...
            try {
                const { error } = await supabaseClient.from('posts').insert([{ 
                    author: App.ProfileState.getCurrentName(), 
                    role: '[]', 
                ...
                if (error) throw error;
                composerAttachedImageBase64 = null; composerAttachedFileBase64 = null;
        ...
        async toggleLike(id, currentLikes, currentDislikes) {
            ...
            if (likedLogs.includes(id)) {
                ...
                if (dislikedLogs.includes(id)) {
                    ...
                    updatePayload.dislikes = Math.max(0, currentDislikes - 1);
                }
        ...
        async toggleDislike(id, currentLikes, currentDislikes) {
            ...
            if (dislikedLogs.includes(id)) {
                ...
                let updatePayload = { dislikes: currentDislikes + 1 };
                if (likedLogs.includes(id)) {
                    likedLogs = likedLogs.filter(x => x !== id);
